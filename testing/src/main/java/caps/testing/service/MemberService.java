...
public class MemberService {
    ...
    public Long join_worker(MemberSignUpRequestDto memberSignUpRequestDto) throws MemberException {
        ...
        Member manager = memberRepository.findAll().stream()
                .filter(m -> authentication_code.equals(m.getAuthentication_code()))
                .findFirst()
                .orElseThrow(() -> new IllegalStateException("코드를 다시 확인해주세요"));
    ...
    public Long join_manager(ManagerSignUpDto managerSignUpDto) throws Exception {
        ...
        try {
            ...
            location = fetchData(url);
        } catch (Exception e) {
            log.warn("주소 API 호출 실패, 기본값 사용: {}", e.getMessage());
            location = List.of(0.0, 0.0); // 임시 기본값
        }
